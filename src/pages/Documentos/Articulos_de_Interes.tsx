...
export default function Articulos_de_interes() {
  ...
  const handleGuardarBoletin = async () => {
    ...
    try {
      ...
      setMessage(
        `${t('SeAgregoArticuloInteres')} "${boletinTitle}" ${t('ConExito')}`
      );
      setInfoModal(true);
      mutate();
    } catch (err) {
  ...
  const articulosFiltrados = boletines
  ...
  const listaParaRender = isReversed
    ? articulosFiltrados.slice().reverse()
    : articulosFiltrados;
          ...
          <div className="font">
              ...
              >
                <div style={{ width: '50%' }}>
                  <InputGroup size="sm">
                    ...
                    <FormControl
                      placeholder={t('Buscar Articulos de Interes')}
                      aria-label="Buscar artículos de interés"
                      value={searchTerm}
            ...
            <div className="card-gallery pt-0 Documentos px-5">
              {listaParaRender.length === 0 ? (
                <p className="text-center">{t('NoArticulosInteres')}</p>
              ) : (
